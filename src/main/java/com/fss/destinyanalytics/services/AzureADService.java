...
import com.fss.destinyanalytics.config.Config;
...
public class AzureADService {
	 ...
	 * @return AccessToken
	 */
	public static String getAccessToken(ConfigProperties properties)
			throws MalformedURLException, InterruptedException, ExecutionException {

		if (properties.getAuthenticationType().equalsIgnoreCase("MasterUser")) {
			return getAccessTokenUsingMasterUser(properties.getClientId(),
					properties.getPbiUsername(),properties.getPbiPassword());
		} else if (properties.getAuthenticationType().equalsIgnoreCase("ServicePrincipal")) {

			// Check if Tenant Id is empty
			if (properties.getTenantId().isEmpty()) {
				throw new RuntimeException("Tenant Id is empty");
			}
			return getAccessTokenUsingServicePrincipal(properties.getClientId(),
					properties.getTenantId(), properties.getAppSecret());
		} else {

			// Authentication Type is none of the above
			throw new RuntimeException("Invalid authentication type: " + properties.getAuthenticationType());
		}
	}
	...
	private static String getAccessTokenUsingMasterUser(String clientId, String username, String password)
		...
		PublicClientApplication app = PublicClientApplication.builder(clientId)
				.authority(Config.authorityUrl + "organizations") // Use
